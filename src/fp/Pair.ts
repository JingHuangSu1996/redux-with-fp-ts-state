...
import { semigroupOrdering } from 'fp-ts/lib/Ordering';
...
export const getSemigroup = <A>(
  ...
  return {
    concat: (x, y) =>
      new Pair(S.concat(x.fst, y.fst), S.concat(x.snd, y.snd)),
  };
};
...
const traverse =
  <F>(F: Applicative<F>) =>
  <A, B>(ta: Pair<A>, f: (a: A) => HKT<F, B>): HKT<F, Pair<B>> => {
    return F.ap(
      F.map(f(ta.fst), (b1: B) => (b2: B) => new Pair(b1, b2)),
      f(ta.snd),
    );
  };
...
const sequence =
  <F>(F: Applicative<F>) =>
  <A>(ta: Pair<HKT<F, A>>): HKT<F, Pair<A>> => {
    return F.ap(
      F.map(ta.fst, (a1: A) => (a2: A) => new Pair(a1, a2)),
      ta.snd,
    );
  };
 ...
 * @since 1.0.0
 */
export const pair: Applicative1<URI> & Comonad1<URI> = {
  URI,
  map,
  of,
  ap,
